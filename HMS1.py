# To develop Hotel Management System
import pandas as pd
...
def menu():
    ...
    while True:
        ...
        if ch==1:
            ...
            str="THANK YOU VERY MUCH FOR USING HOTEL MANAGEMENT SYSTEM. EXITING....."
            for k in str:
                print(k,end='')
                time.sleep(0.1)
            break
...
def del_rooms():
    ...
    while ans=='y' or ans=='Y':
        ...
        if rno in dfrooms['roomno'].values:
            response=input("Do you really want to remove this Room's data(Y/N)?")
            if response=='y' or response=='Y': 
                dfrooms.drop(dfrooms[dfrooms['roomno']==rno].index,inplace=True)
                print("Room No -",rno,"has been deleted successfully...")
        else:
            print("Room is not found...")
            
        dfrooms.to_csv("rooms.csv",index=False)
        ans=input("Do you want to delete more rooms? (Y/N) ")
    print(dfrooms) 
    ...
    while ans=='y' or ans=='Y':
        ...
        if cid in dfcust['cust_id'].values:
            ...
            if ans=='y' or ans=='Y':   
                ...
                print("Customer id -",cid,"has been deleted successfully...")
        else:
...
def edit_customer():
    ...
    while ans=='y' or ans=='Y':
        ...
        if cid in dfcust['cust_id'].values:
            ...
            print("Customer id -",cid,"has been updated successfully...")
...
def book_room():
    ...
    while ans=='y' or ans=='Y':
        ...
        if cid in dfcust['cust_id'].values:
            print(dfcust.loc[dfcust.cust_id==cid,['cust_name','Age']])
            if rno in dfrooms['roomno'].values:
                stat=dfrooms.loc[dfrooms['roomno'] == rno, 'status'].values[0]
                if (stat=='V'):
                    ans=input("Do you want to check-in (y/n)?")
                    dt_bk=input("Please Enter date of booking/check-in(dd/mm/yyyy): ")
                    if ans.upper()=='Y':
                        dt_chkin=dt_bk
...
def check_out():
    ...
    while ans=='y' or ans=='Y':
        ...
        cond1=((dftrans.cust_id==cid) & (dftrans.roomno==rno) & (dftrans.dt_checkout.isnull()))
        ...
        if cond1.any() and cond2.any():
            ...
            dftrans.loc[(dftrans['cust_id'] == cid) & (dftrans['roomno'] == rno) & (dftrans['dt_checkout'].isnull()), 'dt_checkout']=dt_out
            dfrooms.loc[dfrooms.roomno == rno, 'status']='V'
            chrg=dfrooms.loc[dfrooms.roomno==rno,'charge'].values[0]
            dt_in=dftrans.loc[(dftrans['cust_id'] == cid) & (dftrans['roomno'] == rno), 'dt_checkin'].values[0]
            ...
            mbill=n_days*chrg
            dftrans.loc[(dftrans['cust_id'] == cid) & (dftrans['roomno'] == rno), 'days']=n_days
            dftrans.loc[(dftrans['cust_id'] == cid) & (dftrans['roomno'] == rno), 'bill']=mbill
            ...
            print("Room is already Vacant or Billing/check out already done")
        ...
        if ch==1:
            ...
            for i in range(length):
                if c_nm in dfcust.cust_name[i]:
                    print(dfcust.cust_id[i],"-->",dfcust.cust_name[i],"-->",dfcust.Address[i])
        elif ch==7:
...
def visualization():
    ...
    while True:
        ...
        print("          *************************************")
        print()
        print("           1- Room's charges")
        print("           2- Customer's Age (Specific Group)")
        print("           3- Customers Stay & Billing in the Hotel")
        ...
        if ch==1:
            ...
            if typ=='L':
                x1=dfrooms.roomno
                y1=dfrooms.charge
                n=np.arange(len(dfrooms))        
                plt.plot(n, y1,color='m', linestyle='-.',linewidth=2,marker='o')
                plt.xticks(n,x1,rotation=25)
            elif typ=='B':
                dfrooms.plot('roomno','charge', kind='bar', color='r',rot=20)
            plt.title("Plot: Room no vs Charge (INR)",fontsize=16,color='r')
            plt.xlabel("Room no.-->",fontsize=12,color='m')
            plt.ylabel("Charges (INR)-->",fontsize=12,color='m')
            plt.grid()    
            plt.show()      
        elif ch==2:
            typ=input("Graph Type [Line-L,Bar-B]:")
            if typ=='L':
                x1=dfcust.cust_name
                y1=dfcust.Age
                n=np.arange(len(dfcust))        
                plt.plot(n, y1,color='r', linestyle='-.',linewidth=2,marker='^')
                plt.xticks(n,x1,rotation=25)
                
            elif typ=='B':
                dfcust.plot('cust_name','Age', kind='bar', color='m',rot=20)
            plt.title("Plot: Customer Name vs Age",fontsize=16,color='m')
            plt.xlabel("Customer's Name-->",fontsize=12,color='b')
            plt.ylabel("Age-->",fontsize=12,color='b')
            plt.grid()    
            plt.show()
        elif ch==3:
            df2=dftrans.groupby('cust_id', as_index=False).agg({"bill": "sum","days":"sum"})
            #df2=dftrans.groupby('cust_id')['bill'].sum()
            #df2.plot(kind='bar',color='g', rot=30)
            choice=int(input("Enter Customer wise stay in Hotel ( 1-days, 2-billing):"))
            if choice==1:
                df2.plot('cust_id','days',kind='bar',color='g', rot=30)
                plt.title("Plot: Customer wise No.of days \nStay in Hotel",fontsize=16,color='g')
                plt.xlabel("Customer No-->",fontsize=12,color='b')
                plt.ylabel("No. of Days-->",fontsize=12,color='b')
            elif choice==2:
                df2.plot('cust_id','bill',kind='bar',color='m', rot=30)
                plt.title("Plot: Customer wise Bill Payments (INR)",fontsize=16,color='g')
                plt.xlabel("Customer No-->",fontsize=12,color='b')
                plt.ylabel("Paid Bill (INR)-->",fontsize=12,color='b')
            plt.grid()    
            plt.show()
        elif ch==4:
            print("Going back to Main Menu.....")
